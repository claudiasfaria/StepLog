...
// Adiciona isto aos imports existentes
  ...
  return (
    <div className="grid-bg" style={{ height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: activeTab === "map" ? "flex" : "none", flexDirection: "column", overflow: "hidden" }}>
          ...
          {/* Selected zone strip — in-flow, no dead space */}
          {selectedZone && (() => {
    ...
    return (
      <>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 44 }}>
          ...
          {/* Search + category filter — public/Lisboa only */}
          {isPublic && (
            <div style={{ flexShrink: 0, padding: "10px 16px 0", display: "flex", flexDirection: "column", gap: 8 }}>
              {/* Search input */}
              <div style={{ position: "relative" }}>
                <Icon d={ICONS.mapPin} size={14} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--muted)", pointerEvents: "none" }} />
                <input
                  value={searchQuery}
                  onChange={e => { setSearchQuery(e.target.value); setSelectedZoneId(null); }}
                  placeholder="Pesquisar spots em Lisboa…"
                  style={{
                    width: "100%", boxSizing: "border-box",
                    background: "rgba(255,255,255,0.05)",
                    border: "1px solid rgba(255,255,255,0.09)",
                    borderRadius: 12, padding: "9px 12px 9px 36px",
                ...
                {searchQuery && (
                  <button
                    ...
                    style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: 14, lineHeight: 1, padding: 2 }}
                  >×</button>
                )}
              </div>
              {/* Category chips */}
              <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2 }}>
                <button
                  onClick={() => { setActiveCategory(null); setSelectedZoneId(null); }}
                  style={{
                    flexShrink: 0, padding: "5px 12px", borderRadius: 99,
                    fontSize: 11, fontFamily: "var(--font-body)", fontWeight: 600,
                    border: "1px solid", cursor: "pointer",
                    background: !activeCategory ? "rgba(255,179,71,0.15)" : "rgba(255,255,255,0.04)",
                    borderColor: !activeCategory ? "rgba(255,179,71,0.4)" : "rgba(255,255,255,0.08)",
                    color: !activeCategory ? "#FFB347" : "var(--muted)",
                  }}
                >Todos</button>
                {PUBLIC_CATEGORIES.map(cat => (
                  <button
                    key={cat.id}
                    onClick={() => { setActiveCategory(prev => prev === cat.id ? null : cat.id); setSelectedZoneId(null); }}
                    style={{
                      flexShrink: 0, padding: "5px 12px", borderRadius: 99,
                      fontSize: 11, fontFamily: "var(--font-body)", fontWeight: 600,
                      border: "1px solid", cursor: "pointer",
                      background: activeCategory === cat.id ? "rgba(255,179,71,0.15)" : "rgba(255,255,255,0.04)",
                      borderColor: activeCategory === cat.id ? "rgba(255,179,71,0.4)" : "rgba(255,255,255,0.08)",
                      color: activeCategory === cat.id ? "#FFB347" : "var(--muted)",
                    }}
          ...
          {/* ZONE CARDS */}
          <div style={{ flexShrink: 0, padding: "4px 16px 16px", display: "flex", flexDirection: "column", gap: 6}}>
